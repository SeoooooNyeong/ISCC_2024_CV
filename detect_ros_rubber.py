...
from visualizer import draw_detections_rubbercone
...
class Yolov7Publisher:
    def __init__(self, img_topic: str, weights: str, conf_thresh: float = 0.5,
        ...
        # 내가 원하는 퍼블리셔 선언
        self.rubbercone_publisher = rospy.Publisher("rubbercone", Int32, queue_size=queue_size)
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        if self.visualization_publisher:
            ...
            vis_img = draw_detections_rubbercone(np_img_orig, bboxes, classes,
                                                 self.class_labels)
            vis_msg = self.bridge.cv2_to_imgmsg(vis_img, encoding="bgr8")
            self.visualization_publisher.publish(vis_msg)
            # 제일 위에 있는 신호등 정보로 제어 판단 값 publish
            if len(C_y) > 0:
                if (C_y[0][0] == 2 or C_y[0][0] == 5):  # 직진
                    self.rubbercone_publisher.publish(1)



                elif (C_y[0][0] == 8):  # 좌회전, 직진 둘다
                    self.rubbercone_publisher.publish(2)



                elif (C_y[0][0] == 3 or C_y[0][0] == 7):  # 좌회전
                    self.rubbercone_publisher.publish(3)

                else:  # 정지
                    self.rubbercone_publisher.publish(0)
